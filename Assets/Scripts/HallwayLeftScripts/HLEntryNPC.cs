...
public class HLEntryNPC : MonoBehaviour
{
    ...
    [Tooltip("Unique identifier for this room (e.g., 'Hallway', 'CockpitLeft')")]
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("[HLEntryNPC] Missing UI references!");
            enabled = false;
            return;
        }
    ...
    private bool ValidateUIReferences()
    {
        if (npcCanvas == null)
        {
            Debug.LogError("[HLEntryNPC] NPC Canvas not assigned!");
            return false;
        }

        if (npcText == null)
        {
            Debug.LogError("[HLEntryNPC] NPC Text not assigned!");
            return false;
        }
    ...
    private void CheckFirstEntry()
    {
        ...
        if (visited == 0)
        {
            ...
            Debug.Log($"[HLEntryNPC] First entry to room: {roomID}");
        ...
        {
            Debug.Log($"[HLEntryNPC] Room already visited: {roomID}");
        }
    }
    ...
    public void ResetRoomVisit()
    {
        ...
        Debug.Log($"[HLEntryNPC] Reset room visit for: {roomID}");
    }
