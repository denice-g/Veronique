...
public class RoomEntryNPC : MonoBehaviour
{
    ...
    [TextArea(3, 5)]
    [Tooltip("Message shown after player interacts with the button")]
    public string afterInteractionMessage = "The crisis has begun! Handle it carefully.";
    ...
    [Tooltip("Unique identifier for this room (e.g., 'CrisisRoom', 'CockpitLeft')")]
    public string roomID = "CrisisRoom";
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("[RoomEntryNPC] Missing UI references!");
            enabled = false;
            return;
        }
    ...
    private bool ValidateUIReferences()
    {
        if (npcCanvas == null)
        {
            Debug.LogError("[RoomEntryNPC] NPC Canvas not assigned!");
            return false;
        }

        if (npcText == null)
        {
            Debug.LogError("[RoomEntryNPC] NPC Text not assigned!");
            return false;
        }
    ...
    private void CheckFirstEntry()
    {
        ...
        if (visited == 0)
        {
            ...
            Debug.Log($"[RoomEntryNPC] First entry to room: {roomID}");
        ...
        {
            Debug.Log($"[RoomEntryNPC] Room already visited: {roomID}");
        }
    }
    ...
    public void ShowInteractionMessage()
    {
        ...
        Debug.Log("[RoomEntryNPC] Showing interaction message");
        StartCoroutine(ShowMessage(afterInteractionMessage, interactionSound));
    }
    ...
    [ContextMenu("Test Interaction Message")]
    ...
    public void ResetRoomVisit()
    {
        ...
        Debug.Log($"[RoomEntryNPC] Reset room visit for: {roomID}");
    }
